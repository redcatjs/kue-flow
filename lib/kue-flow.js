  ...
  Object.assign(Job.prototype, {
    ...
    async saveAsync() {
      ...
      if (this.data._unique) {
        ...
        if (id) {
          job = await this.QueueJob.getAsync(id)
          if (job) {
            ...
            switch(job._state){
              ...
              case 'active':
                if(uniqueKillActive){
                  ...
                  if(mpid){
                    try{
                      await this.QueueJob.queue.killMPid(mpid)
                      // console.log('killed job', this.data._unique, mpid)
                    }
                    catch(e){
                    ...
                    console.error('error mpid not found for unique job '+job.id+' of type '+job.type)
                  }
                }
                job = this
                break
    ...
    async getUniqueJobMPid() {
      if(this.data._unique){
        return this.client.hgetAsync(this.QueueJob.queue.uniqueMPidKeyNS(), this.data._unique)
      }
    },
